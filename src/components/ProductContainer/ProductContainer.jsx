...
export const ProductsContainer = () => {
        ...
        if (page * 16 < catalog.length) {
            return (
                <p className={styles.products_amount}>
                    Показано {page * 16} из {catalog.length} товаров
                </p>
            )
        }
        else {
            return (
                <p className={styles.products_amount}>
                    показано {catalog.length} из {catalog.length} товаров
                </p>
            )
        }
    }
    ...
    return (
        <>
            ...
            <ProductsShown/>
            <Pagination 
                totalPages = {catalog.length % 16 == 0 ? catalog.length / 16 : Math.floor(catalog.length / 16) + 1}
